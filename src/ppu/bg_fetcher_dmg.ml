...
module State = struct
  ...
  let all = [Idle; Init_coords; Fetch_tile_no_1; Fetch_tile_no_2; Fetch_tile_low_1; Fetch_tile_low_2; Fetch_tile_high_1; Fetch_tile_high_2; Push_pixels; Done_state]
end
...
let create _scope (i : _ I.t) =
  ...
  let pixel_in_tile = Always.Variable.reg spec ~width:3 in (* 0-7 pixels within tile *)
  let pixel_count = Always.Variable.reg spec ~width:3 in (* Debug counter *)
  
  (* Timing control variables *)
  let fetch_cycle = Always.Variable.reg spec ~width:1 in (* 2-cycle timing for fetch states *)
  let initial_fetch_done = Always.Variable.reg spec ~width:1 in (* Track initial 12-cycle delay *)
  let first_pixel = Always.Variable.reg spec ~width:1 in (* Track if this is the very first pixel *)
  ...
  (* let tile_number = lsb (tile_x.value ^: tile_y.value) in *) (* XOR pattern: 0 or 1 *)
  ...
  let blk_x = uresize (srl x.value Constants.checker_shift_bits) Constants.rgb555_channel_width in
  let blk_y = uresize (srl y.value Constants.checker_shift_bits) Constants.rgb555_channel_width in
  ...
  (* State machine logic *)
  Always.(compile [
    busy <-- (sm.is Init_coords |: sm.is Fetch_tile_no_1 |: sm.is Fetch_tile_no_2 |: sm.is Fetch_tile_low_1 |: sm.is Fetch_tile_low_2 |: sm.is Fetch_tile_high_1 |: sm.is Fetch_tile_high_2 |: sm.is Push_pixels);
    ...
    sm.switch [
      Idle, [
        ...
        when_ i.start [
          ...
          y <--. 0;
          pixel_in_tile <--. 0;
          pixel_count <--. 0; (* Initialize debug counter *)
          fetch_cycle <--. 0;
          initial_fetch_done <--. 0;
          first_pixel <--. 1; (* Mark that we need to write the first pixel at (0,0) *)
      ...
      Init_coords, [
        fb_a_we <-- gnd;
        (* Wait one cycle for coordinate initialization, then start fetch pipeline *)
      ...
      Fetch_tile_no_2, [
        fb_a_we <-- gnd;
        sm.set_next Fetch_tile_low_1
      ];

      (* 2-cycle fetch tile low *)
      Fetch_tile_low_1, [
        fb_a_we <-- gnd;
        sm.set_next Fetch_tile_low_2
      ];
      
      Fetch_tile_low_2, [
        fb_a_we <-- gnd;
        sm.set_next Fetch_tile_high_1
      ];

      (* 2-cycle fetch tile high *)
      Fetch_tile_high_1, [
        fb_a_we <-- gnd;
        sm.set_next Fetch_tile_high_2
      ];
      
      Fetch_tile_high_2, [
        fb_a_we <-- gnd;
        (* First time through, repeat the fetch cycle for 12-cycle initial delay *)
        if_ (initial_fetch_done.value ==:. 0) [
          ...
          pixel_count <--. 0; (* Reset debug counter *)
          (* Don't reset coordinates - they should continue from where they left off *)
      ...
      Push_pixels, [
        ...
        if_ (addr_pix ==:. total_pixels_minus_1) [
          sm.set_next Done_state
        ] [
          (* Always advance coordinates for next pixel *)
          x <-- x.value +:. 1;
          when_ (x.value ==:. (Constants.screen_width - 1)) [
            x <--. 0;
            y <-- y.value +:. 1
          ];
