...
let test_reset_start_coincidence () =
  ...
  printf "  During reset: busy=%b, done=%b, we=%b\n" 
    busy_during_reset done_during_reset we_during_reset;
  ...
  let cycle_count = ref 1 in (* We already cycled once after start *)
  ...
  while Bits.to_bool !(outputs.busy) do
    ...
    if is_done_now then begin
      Int.incr done_pulse_count;
      (* The done pulse should occur on the same cycle as the last write,
         so busy should still be high on that cycle. This is correct behavior. *)
    end;
  ...
  let expected_cycles = 160 * 144 + 1 in (* 23040 total pixels + 1 start cycle *)
  ...
  for _i = 1 to 5 do
    ...
    if busy || done_ || we then
      failwith "FAIL: FSM should remain idle after completion";
  done;
  ...
  Hardcaml_waveterm.Waveform.print ~wave_width:1 
    ~display_width:70 ~display_height:20 waves;
