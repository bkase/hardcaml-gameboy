...
open Alcotest
open Gb_shared
module Pixels = Spec_ppu.Gb_math_int.Pixels
module Palette = Spec_ppu.Gb_math_int.Palette
module Addressing = Spec_ppu.Gb_math_int.Addressing

let test_rgb555_pack_unpack () =
  (* Test known constants *)
  let red = Pixels.pack_rgb555 ~r5:31 ~g5:0 ~b5:0 in
  check int "red packed correctly" 0x7C00 red ;

  let green = Pixels.pack_rgb555 ~r5:0 ~g5:31 ~b5:0 in
  check int "green packed correctly" 0x03E0 green ;

  let blue = Pixels.pack_rgb555 ~r5:0 ~g5:0 ~b5:31 in
  check int "blue packed correctly" 0x001F blue ;

  let white = Pixels.pack_rgb555 ~r5:31 ~g5:31 ~b5:31 in
  check int "white packed correctly" 0x7FFF white ;

  let black = Pixels.pack_rgb555 ~r5:0 ~g5:0 ~b5:0 in
  check int "black packed correctly" 0x0000 black ;

  (* Test round-trip *)
  let r5, g5, b5 = Pixels.unpack_rgb555 0x03E0 in
  check int "green unpack R5" 0 r5 ;
  check int "green unpack G5" 31 g5 ;
  check int "green unpack B5" 0 b5 ;

  (* Test channel masking *)
  let overflow = Pixels.pack_rgb555 ~r5:63 ~g5:63 ~b5:63 in
  let r5', g5', b5' = Pixels.unpack_rgb555 overflow in
...
let test_rgb888_conversion () =
  ...
  let r8, g8, b8 = Spec_ppu.Gb_math_int.to_rgb888 ~rgb555:0x7FFF in
  check int "white R8" 255 r8 ;
  check int "white G8" 255 g8 ;
  check int "white B8" 255 b8 ;

  (* Test black conversion *)
  let r8, g8, b8 = Spec_ppu.Gb_math_int.to_rgb888 ~rgb555:0x0000 in
...
let test_gb_addressing () =
  ...
  let eff_y, tile_row, map_row, tile_x0, fine_x =
    Addressing.scroll_decompose ~ly:0 ~scy:0 ~scx:0
  in
  ...
  (* Test map addressing *)
  check int "map addr (0,0)" 0x1800 (Addressing.map_addr_local ~map_row:0 ~tile_x:0) ;

  (* Test tile row addressing *)
  let lo_addr, hi_addr = Addressing.tile_row_addrs_local ~tile_index:0 ~tile_row:0 in
...
let test_gb_palette () =
  ...
  check int "color 0 -> white" white
    (Palette.dmg_index_to_rgb555 ~idx:0 ~white ~light ~dark ~black) ;
  check int "color 3 -> black" black
    (Palette.dmg_index_to_rgb555 ~idx:3 ~white ~light ~dark ~black) ;

  (* Test BGP application *)
  let bgp = 0b11100100 in
  (* 3,2,1,0 -> black,dark,light,white *)
  check int "BGP color 0" 0 (Palette.apply_bgp ~palette:bgp ~color_index:0) ;
  check int "BGP color 1" 1 (Palette.apply_bgp ~palette:bgp ~color_index:1) ;
  check int "BGP color 2" 2 (Palette.apply_bgp ~palette:bgp ~color_index:2) ;
  check int "BGP color 3" 3 (Palette.apply_bgp ~palette:bgp ~color_index:3)
